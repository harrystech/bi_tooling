...
class Verify:
    def get_token_client(self):
        ...
        client = looker.ApiClient(
            os.environ.get("LOOKER_BASE_URL"),
            "Authorization",
            "token " + token.access_token,
        )
        return token, client

    def get_fields(self, model_explore_body):
        """
        Create a list of dimensions and measures from the given model-explore
        """
        dimensions = [
            dimension.name for dimension in model_explore_body.fields.dimensions
        ]
        measures = [measure.name for measure in model_explore_body.fields.measures]
        return dimensions + measures

    def check_for_query_error(self, query_id, token, timeout=3):
        """
    ...
    def process_branch(
        ...
        """
        # Generate Query-ID
        query_id = self.generate_query_id(query_client, branch)

        # Check for query error
        message = self.check_for_query_error(query_id, token)
        ...
        if message and IGNORABLE_SQL_ERROR not in message:
            ...
            # Else split the field into two and put both back on the queue
                ...
                divided_branches = self.divide_branch(branch)
    ...
    def divide_branch(self, branch):
        ...
        return [
            [model_name, explore_name, left_fields, starting_field_count],
            [model_name, explore_name, right_fields, starting_field_count],
        ]

    def generate_query_id(self, query_client, branch):
        """
    ...
    def run(self):
        ...
        def is_checkable(model):
            return model.has_content and (
                not selected_models or model.name in selected_models
            )
        ...
        for model in models:
            ...
            for explore_info in model.explores:
                ...
                model_explore_body = model_client.lookml_model_explore(
                    model.name, explore_info.name
                )
                model_explore_fields = self.get_fields(model_explore_body)
                branch_queue.put(
                    [
                        model.name,
                        explore_info.name,
                        model_explore_fields,
                        len(model_explore_fields),
                    ]
                )

        total_starting_field_count = np.sum(
            [count[-1] for count in list(branch_queue.queue)]
        )
        ...
        while True:
            try:
                ...
                model_name, explore_name, field, starting_field_count = branch
                logging.info(
                    "Error field: '{}' explore :'{}'".format(field[0], explore_name)
                )
            except queue.Empty:
                break
